...
$orders = "Common", "Dev", "Finances", "Sports", "Accounts","Shopping"


#Write-Output $orders[0]
#Exit

$projectDir = "M:\Seafile\Dev\Projects\Github_Pages"
$bookmarksFolder = "C:\Users\Mike\OneDrive\Bookmarks"
$outputBookmarksFile = "$projectDir\bookmarks.html"
cd $bookmarksFolder

$head = Get-Content "$projectDir\head_home.txt"

function parseURLFile ($item, $bookmarksFolder) {
    $dir = Split-Path (Split-Path $item -Parent) -Leaf
    $linktext = $item.Replace(".URL", "").Replace($bookmarksFolder,"").Replace("\","")
...
function recurseDirForURLFiles($items) {
    ForEach ($item in $items) { 
        If(Test-Path $item.FullName -pathtype container){
            ...
            parseURLFile $item.FullName $bookmarksFolder 
        }
    }
}

$items = Get-ChildItem $bookmarksFolder | select FullName
...
For ($i=0; $i -le $orders.length; $i++) {
    $orderedContainers.Add($orders[$i])
    }
...
Write-Output $outputBookmarksFile

ConvertTo-HTML -head $head -body $body | Out-File $outputBookmarksFile
##Write-Debug $body